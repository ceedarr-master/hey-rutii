  ...
  return `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:var(--space-12); width:100%;">
      ...
      <div style="display:flex; gap:12px;">
        ...
        <button class="btn-xs btn-tertiary btn-icon" onclick="window.goScreen('list')">${getSfSymbol("xmark", 16)}</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-group-body">
        <div class="intro-list">
          ...
          <div class="intro-list-item">${getSfSymbol("stopwatch", 16, "var(--text-brand-accent)")} 시간 진행 단계는 자동 카운트다운 + 종료 시 알림음</div>
          <div class="intro-list-item">${getSfSymbol("123.rectangle", 16, "var(--text-brand-accent)")} 횟수 진행 단계는 직접 "다음" 버튼으로 넘기기</div>
        </div>
      </div>
